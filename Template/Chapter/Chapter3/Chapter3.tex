...
\section{Đặt vấn đề}
Ngăn chặn sự phát tán của thông tin sai lệch là vấn đề đang được nhiều nhà khoa học quan tâm trong thời gian gần đây. Zhang \cite{zhang1} để xuất bài toán tìm số nút nhỏ nhất trong khoảng cách $\delta$ (vùng N$_{\delta}$ ()) đối với nguồn phát thông tin sai lệnh để vô hiệu hóa sao cho thông tin sai lệnh đến nút đích $r$ được giới hạn. Bên cạnh đó, một số tác giả đề xuất giải pháp hạn chế sự lan truyền thông tin sai lệch trên mạng xã hội bằng cách chọn ra một số đỉnh ban đầu để tiêm thông tin tốt, từ đó lan truyền những thông tin này trên cùng mạng nhằm thuyết phục những người dùng khác tin theo, trong đó sử dụng các mô hình lan truyền thông tin khác nhau \cite{nguyen9}, \cite{nguyen30}.
Tuy vậy các nghiên cứu trên có các nhược điểm sau:
\begin{itemize}
	\item Sử dụng các mô hình phát tán thông tin là mô hình xác suất mà hàm mục tiêu là hàm $\#P -$ Khó.
	
	\item Các nghiên cứu này đều tìm với ngân sách cho trước, chưa xét tới trường hợp tìm số nút nhỏ nhất để dập tắt sự phát tán của thông tin sai lệch. Trong thực tế trong nhiều trường hợp chúng ta không biết cần bao nhiêu chi phí để loại bỏ được sự phát tán của thông tin sai lệch.
	
	\item Các nghiên cứu trên chưa thực hiện được đối với bài toán ngăn chặn sự phát tán thông tin sai lệch tại Việt Nam.
\end{itemize}
...
\subsection{Mô hình lan truyền thông tin Ngưỡng tuyến tính với bước thời gian rời rạc}
...
Trong mô hình LT, giá trị của ngưỡng $\theta_{v}$ được đặt một cách ngẫu nhiên trong khoảng $[0,1]$ và sẽ được chỉnh sửa dựa vào thông tin có được thêm trong quá trình lan truyền. Do đó, mô hình này thuộc dạng mô hình ngẫu nhiên. Khác với mô hình LT, trong mô hình T-DLT ngưỡng lây nhiễm $\theta_{v}$ được cho trước. Trường hợp này, giá trị ngưỡng có thể được đặt dựa theo các khảo sát thực tế hoặc các phương pháp khai phá dữ liệu \cite{zaixin}. So sánh với mô hình DLT, mô hình T-DLT mà nhóm tác giả đề xuất có khía cạnh mới như việc quá trình lan truyền sẽ ngừng lại khi số bước lan truyền đạt ngưỡng $d$. Mô hình DLT chính là trường hợp đặc biệt của mô hình T-DLT khi $d=|V|$.
...
Kí hiệu $f_{d}(I)$ là tập hợp các đỉnh đã bị lây nhiễm sau $d$ vòng trên đồ thị $G = (V,E)$ trong mô hình T-DLT, $f_{d}(I,A)$ là tập các đỉnh bị lây nhiễm sau khi loại bỏ một tập các đỉnh $A \subseteq V \setminus I$ từ $G$ (số đỉnh bị lây nhiễm trong đồ thị sót lại). Khi đó, số đỉnh cứu được khi loại bỏ tập $A$ là:

\begin{align}
h_{d}(A)=| f_{d}(I,\emptyset)|-|f_{d}(I,A)|
\end{align} 
...
Một cách trực quan, số lượng gia tăng các đỉnh được cứu $\delta(A, u)$ có thể xấp xỉ theo $\beta(u)$. Bên cạnh đó, để tăng cường tính hiệu quả của thuật toán đề xuất, nhóm tác giả kết hợp $\alpha(u)$ và $\beta(u)$ để tính toán khả năng lan truyền tin sai lệch của đỉnh $u$. Ý tưởng chính của thuật toán là chọn ra các đỉnh một cách lần lượt theo đánh giá của hai hàm $\alpha$ và $\beta$. Ban đầu, nhóm tác giả khởi tạo tập đỉnh được chọn $A=\emptyset$ và đặt tập các đỉnh được xét chọn $U$ bằng với $V_{d}$. Trong mỗi bước, nhóm tác giả chọn ra đỉnh có trị số $\beta(u)$ lớn nhất trong đồ thị còn lại. Trường hợp tất cả các đỉnh đều có giá trị $\beta(u)$ là 0, nhóm tác giả chọn đỉnh có giá trị $\alpha(u)$ cực đại. Chi tiết thuật toán được mô tả trong Thuật toán \ref{alg:Algo1}.
...
Để đánh giá độ phức tạp của thuật toán này, chúng ta cần đánh giá trước độ phức tạp của các thuật toán \ref{alg:Algo1}, \ref{alg:Algo2}, \ref{alg:Algo3}. Nhóm tác giả nhận thấy rằng việc tính  toán $f_{d}(I)$ (Thuật toán \ref{alg:Algo2}, dòng 7 - 22) có cách thức hoạt đông giống BFS, đồng thời quá trình thực hiện thuật toán chỉ xét đến đồ thị $G_{d}$, do đó, độ phức tạp của nó là $O(m_{d} + n_{d})$. Với bước tính toán $\alpha(u)$ và $\beta(u)$ (Thuật toán \ref{alg:Algo3}, dòng 1 - 11), cũng do quá trình thực hiện thuật toán chỉ xét đến đồ thị $G_{d}$, ta thấy độ phức tạp của nó là $O(m_{d} + n_{d})$. Do đó, độ phức tạp chung của thuật toán là $O(m_{d} + n_{d})$. 
...
Nhóm tác giả thực nghiệm thuật toán trên nhiều bộ dữ liệu thực tế khác nhau. Bên cạnh việc lựa chọn các bộ dữ liệu để đảm bảo sự đa dạng về kích thước, nhóm tác giả cũng lựa chọn nhiều miền dữ liệu khác nhau, bảng \ref{tab:Table1} thể hiện các bộ dữ liệu nhóm tác giả sử dụng.
\begin{table}
	\centering
	\begin{tabular}{|c|c|c|c|c|}
		...
		\hline 
		\textbf{Gnutella \cite{lesk19}} & 6,301 & 20,777 & Có hướng & 3.29 \\ 
		\hline 
		\textbf{Wikipedia vote \cite{lesk18}} & 7,115 & 103,689 & Có hướng & 14.57 \\ 
		\hline 
		\textbf{Amazon \cite{lesk16}} & 262,111 & 1,234,877 & Có hướng & 4.71 \\ 
		\hline 
		\textbf{Google web \cite{lesk20}} & 875,713 & 5,105,039 & Có hướng & 5.83 \\ 
		\hline 
	\end{tabular} 
