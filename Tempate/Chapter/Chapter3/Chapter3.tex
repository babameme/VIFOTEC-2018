...
\end{theo}
\begin{proof}
	Để chứng minh TMB là $\#P-hard$ chúng tôi sẽ chỉ ra độ khó của bài toán TMB là lớn hơn hoặc bằng với độ khó của bài toán $s$-$t$ paths mà đã được chứng minh trong \cite{vali} là có độ khó $\#P-hard$. 
...
Vậy nên toàn bộ thuật toán TMB có độ phức tạp là O($\mu$(m+qn)).
	...
	Trong phần này nhóm tác giả đưa ra kết quả thực nghiệm của giải hai giải pháp chỉ ra trong mục trước trên dữ liệu có sẵn và dữ liệu thu thập được, từ đó để thấy được hiệu quả của phương pháp nhóm đề xuất.
		...
		\subsubsection{Giải pháp Targeted Misinformation Blocking (Xác định và ngăn chặn thông tin sai lệch trên mạng xã hội)}
		{\itshape Dữ liệu có sẵn}	
		...
		{\itshape - Kết quả thực nghiệm:} Như mô tả trong hình  
